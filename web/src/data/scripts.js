...
export const scripts = [
  ...
  },
  {
    ...
    description: 'One-command install: detects platform, checks deps, clones, builds, and deploys Llama.cpp.',
    longDescription: 'Platform-aware installation script that detects your OS and hardware, checks for required dependencies (advising on how to install missing ones), clones the official Llama.cpp repository, compiles with hardware-specific backends, and installs to your home directory. No sudo required.',
    icon: IconInstall,
    iconBg: 'bg-mint-50',
    ...
    features: [
      ...
      'Dependency checking with install instructions for missing packages',
      'CUDA, ROCm, Vulkan, and Metal backend support',
      'User-local installation (~/.local/llama-cpp)',
    ...
    params: [
      ...
      { id: 'GPU_BACKEND',        label: 'GPU backend',         type: 'select', options: ['auto', 'cuda', 'rocm', 'vulkan', 'sycl', 'opencl', 'cpu'], envVar: true, desc: 'GPU acceleration backend. auto = detect from installed drivers' },
      { id: 'CUDA_ARCHITECTURES', label: 'CUDA architectures',  type: 'text',   placeholder: '86;89;90;120',                             envVar: true, desc: 'Override CUDA compute capabilities (e.g. 86;89;90). Leave blank to auto-detect from nvcc version' },
    ...
    env: [
      ...
      { name: 'BUILD_DIR', default: '~/.local/llama-cpp/build', desc: 'CMake build directory' },
      { name: 'LOG_FILE', default: '~/.local/llama-cpp/logs/install.log', desc: 'Installation log file' },
    ],
    examples: [
      { label: 'Standard installation', code: './scripts/install/install-lamacpp.sh' },
      { label: 'Custom install directory', code: 'INSTALL_DIR=~/llama ./scripts/install/install-lamacpp.sh' },
    ],
  },
  {
    id: 'compile',
    ...
    env: [
      ...
      { name: 'INSTALL_DIR', default: '~/.local/llama-cpp', desc: 'Final installation directory' },
      { name: 'LOG_FILE', default: '~/.local/llama-cpp/logs/compile.log', desc: 'Compilation log file' },
    ],
    examples: [
  ...
  },
  {
    ...
    longDescription: 'Upgrade your existing Llama.cpp installation safely. Creates a timestamped backup of your current installation, pulls the latest source, recompiles with hardware detection, and rolls back automatically if anything goes wrong. No sudo required.',
    icon: IconUpgrade,
    iconBg: 'bg-orange-50',
    ...
    features: [
      ...
      'Graceful stop of user-owned llama-server processes',
  ...
  },
  {
    ...
    env: [
      ...
      { name: 'LOG_DIR', default: '~/.local/llama-cpp/logs', desc: 'Log directory' },
