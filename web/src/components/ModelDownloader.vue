...
<template>
  <div class="max-w-3xl mx-auto px-8 py-8 space-y-8">
    ...
    <div class="flex items-start gap-4">
      <div
        class="w-12 h-12 rounded-xl bg-violet-50 flex items-center justify-center shrink-0"
      >
        <svg
      ...
      <div>
        ...
        <p class="text-sm text-gray-400 mt-0.5">
          Search HuggingFace and download GGUF models directly
        </p>
      </div>
    </div>
    ...
    <div>
      ...
      >
        ...
        <span class="normal-case font-normal text-gray-400 ml-1"
          >(optional — required for gated models)</span
        >
      </label>
      <input
        :value="hfToken"
    ...
    <div>
      ...
      <div class="flex gap-2">
        ...
        >
          <span v-if="searching" class="flex items-center gap-2">
            <span
              class="w-3 h-3 border-2 border-violet-300 border-t-white rounded-full animate-spin inline-block"
            ></span>
            Searching
          </span>
      ...
      <p v-if="searchError" class="mt-2 text-xs text-red-500">
        {{ searchError }}
      </p>
    </div>
    ...
    <div v-if="results.length" class="space-y-2">
      ...
      >
        ...
        >
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-gray-400 mt-0.5 flex items-center gap-3">
              <span v-if="model.downloads"
                >⬇ {{ model.downloads.toLocaleString() }}</span
              >
              <span v-if="model.likes"
                >♥ {{ model.likes.toLocaleString() }}</span
              >
        ...
        >
          ...
          >
            <span
              class="w-3 h-3 border-2 border-gray-300 border-t-gray-500 rounded-full animate-spin inline-block"
            ></span>
            Loading variants...
          </div>
          <p v-else-if="filesError" class="px-4 py-3 text-xs text-red-500">
            {{ filesError }}
          </p>
          <p
            ...
            >
              ...
              <span
                ...
                :class="{
                  'bg-emerald-50 text-emerald-700':
                    quantTier(variant.quant) === 'high',
                  'bg-violet-50 text-violet-700':
                    quantTier(variant.quant) === 'mid',
                  'bg-gray-100  text-gray-500':
                    quantTier(variant.quant) === 'low',
                }"
                >{{ variant.quant }}</span
              >
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-gray-400 mt-0.5 flex items-center gap-2">
                  ...
                  <span v-if="variant.sharded" class="text-gray-400"
                    >· {{ variant.files.length }} shards</span
                  >
                </p>
              </div>
    ...
    <div>
      <div class="flex items-center justify-between mb-3">
        <div>
          ...
          <p v-if="modelsDir" class="text-xs text-gray-400 font-mono mt-0.5">
            {{ modelsDir }}
          </p>
        </div>
        <button
        ...
        >
          <svg
            class="w-3 h-3"
            :class="loadingLocal ? 'animate-spin' : ''"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            stroke-width="2"
      ...
      <div
        ...
        class="flex items-start gap-2 px-3 py-2.5 rounded-lg bg-blue-50 border border-blue-100"
      >
        <svg
        ...
        <p class="text-xs text-blue-700 leading-relaxed">
          No models downloaded yet. Search and download models above, or use the
          Launch Server script with the --hf flag to download during launch.
        </p>
      </div>
      <div
        v-else
        class="rounded-xl border border-gray-100 overflow-hidden divide-y divide-gray-100"
      >
        <div
